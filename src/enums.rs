...
use crate::randomizer::person::PLAYABLE;
...
pub const EMBLEM_GIDS: [&str; 20] = ["GID_マルス", "GID_シグルド", "GID_セリカ", "GID_ミカヤ", "GID_ロイ", "GID_リーフ", "GID_ルキナ", "GID_リン", "GID_アイク", "GID_ベレト", "GID_カムイ", "GID_エイリーク", "GID_エーデルガルト", "GID_チキ", "GID_ヘクトル", "GID_ヴェロニカ", "GID_セネリオ", "GID_カミラ", "GID_クロム", "GID_リュール"];
pub const GGIDS: [&str; 20] = ["GGID_マルス", "GGID_シグルド", "GGID_セリカ", "GGID_ミカヤ", "GGID_ロイ", "GGID_リーフ", "GGID_ルキナ", "GGID_リン", "GGID_アイク", "GGID_ベレト", "GGID_カムイ", "GGID_エイリーク", "GGID_エーデルガルト", "GGID_チキ", "GGID_ヘクトル", "GGID_ヴェロニカ", "GGID_セネリオ", "GGID_カミラ", "GGID_クロム", "GGID_リュール"];
...
pub const BLACKLIST_ITEMS: [&str; 26] = [
...
fn is_valid_skill(sid: &str) -> bool {
    if let Some(skill) = SkillData::get(sid) {
        ...
        if skill.is_style_skill() || skill.get_flag() & 511 == 0  { return false; }
    }
    return false;
}

fn is_valid_custom_unit(pid: &str) -> i32 {
    if let Some(person) = PersonData::get(pid) {
        let playable_list = PLAYABLE.get().unwrap();
        if let Some(pos) = playable_list.iter().position(|&x| x == person.parent.index) {
            return pos as i32;
        }
    }
    return -1;
        ...
        for line in lines.flatten() {
            ...
            if spilt.len() > 1 {
                ...
                match spilt[0] {
                    "remove_skill" => { 
                        for z in 1..spilt.len() {
                            if let Some(skill) = SkillData::get(&spilt[z]) {
                                ...
                                if  SKILL_BLACK_LIST.lock().unwrap().iter().find(|&x| *x == index).is_none() && skill.get_inheritance_cost() == 0 {
                    ...
                    "custom_unit_ignore" => {
                        if spilt.len() > 1 {
                            let index = is_valid_custom_unit(spilt[1]);
                            if index != -1 { 
                                SET_RECRUITMENT.lock().unwrap().push( (index, -1, false)); 
                                println!("Added Custom Unit #{} to be ignored", index);
                            }
                        }
                    }
                    _ => { },
