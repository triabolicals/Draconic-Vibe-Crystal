...
pub fn do_continious_mode() {
    if GameVariableManager::get_number("G_Continuous") != 0 && !GameVariableManager::get_bool("G_Cleared_M026")  {
        ...
        if flag & 16 != 0 { new_flag = new_flag - 16; }
        if flag & 32 != 0 { new_flag = new_flag - 32; }
...
fn set_next_chapter() {
    ...
    crate::emblem::emblem_gmap_spot_adjust();
    let current_chapter = GameUserData::get_chapter();
    let current_cid = current_chapter.cid.get_string().unwrap();
...
fn bond_frags_from_achievement(this: &AchieveData) -> i32 {
    unsafe {
        let status = achieve_status(this, None);
        if status == 1 || status == 2 {
            achieve_set_status(this, 3, None);
            return achieve_reward(this, None);
        }
        return 0;
    }
}
...
pub fn update_bonds() {
    ...
    unsafe {
        for x in 0..19 {
            ...
            let god_unit = try_get_god(god, true, None);
            let mut max_level = 1;
            ...
            if god_unit.is_some(){
                ...
                for ff in force_type {
                    let force_iter = Force::iter(Force::get(ff).unwrap());
                    for unit in force_iter {
                        let god_bond = get_bond(g_unit, unit, None);
                        if god_bond.is_some() {
                            let g_bond = god_bond.unwrap();
                            if g_bond.level == 4 { level_up_bond(g_bond, None); }   // C Bond
                            if g_bond.level == 19 { level_up_bond(g_bond, None); }  // A Bond
                    ...
                    for unit in force_iter2 {
                        ...
                        if god_bond.is_some() {
                            ...
                            if g_bond.level < max_level {
                                ...
                                for _x in 0..n_levels { level_up_bond(g_bond, None); }
                                g_bond.exp = bond_exp;
                                inherit_apt_from_god(unit, g_unit, None);
                            }   
                        }   
                    }   
                }  
    ...
    for x in 0..12 {    // Emblem Index
        for y in 0..10 {   // Ring Index
            for z in 0..3 {    //Rank Index
                ...
                unsafe {
                    let count = unit_ring_pool_stock_count(ring, None);
                    if count == 0 { continue; }
                    if count >= ring_cost[z] {
                        sub_ring_to_pool(ring.rid, None, ring_cost[z], None);
                        add_ring(ring_list[index + 1].rid); 
                        println!("{} Bond Rings: {} Merged to {}", ring_cost[z], Mess::get(ring_list[index].name).get_string().unwrap(), rank[z]);
                    }
                }
    }    }    }
    for x in 0..ring_list.len()-3 {  unsafe { ring_count += unit_ring_pool_stock_count(ring_list[x], None); } } // getting new count of bond rings
    println!("Total of {} Bond Rings", ring_count);
}
