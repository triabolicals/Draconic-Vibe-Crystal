...
pub fn change_somniel_hub_dispos() {
    ...
    if GameVariableManager::get_number(DVCVariables::CONTINUOUS) >= 4 {
        if let Some(last_chapter_locator) = hub_list.iter_mut()
            .find(|dispos| dispos.locator.contains("LastChapter"))
        {
...
pub fn change_kizuna_dispos() {
    ...
    if let Some(hub_list) = HubDisposData::try_get_mut(cid) {
        if GameVariableManager::get_number(DVCVariables::RECRUITMENT_KEY) > 0 {
            hub_list.iter()
                .filter(|dispos|
                    dispos.get_locator().str_contains("LocatorStory")
                )
                .for_each(|dispos| {
        ...
        if GameVariableManager::get_number(DVCVariables::EMBLEM_RECRUITMENT_KEY) != 0 {
            if chapter.get_prefixless_cid().str_contains("G00") {
                if let Some(emblem) = hub_list.iter_mut()
                    .find(|dispos| dispos.get_aid()
                        .is_some_and(|aid| aid.str_contains("GID_")
                        )
                    )
                {
    ...
    crate::randomizer::RANDOMIZER_STATUS.try_write()
        .map(|mut lock| lock.kizuna_replacements = true).unwrap();
}
