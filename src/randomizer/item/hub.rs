...
impl ConfigBasicMenuItemSwitchMethods for RandomHubItemMod{
    ...
    extern "C" fn custom_call(this: &mut ConfigBasicMenuItem, _method_info: OptionalMethod) -> BasicMenuResult {
        let value =  DVCVariables::get_exploration_items();
        let result = ConfigBasicMenuItem::change_key_value_i(value, 0, 3, 1);
        if value != result {
            DVCVariables::set_exploration_items(result);
...
pub extern "C" fn vibe_hub_items() -> &'static mut ConfigBasicMenuItem {  
    ...
    hub_items.get_class_mut().get_virtual_method_mut("BuildAttribute")
        .map(|method| method.method_ptr = crate::menus::buildattr::hub_item_build_attr as _);
    hub_items
}
