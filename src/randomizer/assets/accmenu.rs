...
pub fn unit_access_sub_menu_r_call(this: &BasicMenuItem, _method_info: OptionalMethod) -> i32{
    ...
    if GameVariableManager::get_number(DVCVariables::CONTINIOUS) !=  0 {
        if let Some(asset_data) = super::data::ASSET_DATA.get() {
            asset_data.change_accessory(unit, kind, true);
            this.rebuild_text();
            return reload_unit_info(unit);
        }
        else { 0 }
    }
    else if super::accessory::next_unit_accessory(unit, kind, true) {
        this.rebuild_text();
        return reload_unit_info(unit);
    }
    else { return 0x800; }
...
pub fn unit_access_sub_menu_l_call(this: &BasicMenuItem, _method_info: OptionalMethod) -> i32 {
    ...
    if GameVariableManager::get_number(DVCVariables::CONTINIOUS) !=  0 {
        if let Some(asset_data) = super::data::ASSET_DATA.get() {
            asset_data.change_accessory(unit, kind, false);
            this.rebuild_text();
            return reload_unit_info(unit);
        }
        else { 0 }
    }
    else if super::accessory::next_unit_accessory(unit, kind, false) {
        this.rebuild_text();
        return reload_unit_info(unit);
    }
    else { return 0x800; }
...
pub fn unit_access_sub_menu_y_call(this: &BasicMenuItem, _method_info: OptionalMethod) -> i32 {
    if GameVariableManager::get_number(DVCVariables::CONTINIOUS) ==  0 { return 0; } 
    let unit = SortieSelectionUnitManager::get_unit();
    let accessory_list = &mut unsafe { super::unit_get_accessory_list(unit, None) }.unit_accessory_array;
    let rng = Random::get_game();
    let kind = if this.index < 4 { this.index } else { this.index + 1 };
    if let Some(asset_data) = super::data::ASSET_DATA.get() {
        match super::unit_dress_gender(unit) {
            1 => { accessory_list[kind as usize].index = asset_data.male.get_index(kind, rng);},
            2 => { accessory_list[kind as usize].index = asset_data.female.get_index(kind, rng); }
            _ => {}, 
        }
    }
