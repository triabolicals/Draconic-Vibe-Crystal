...
pub fn set_random_bgm_phase(){
    ...
    let string1 = pool.get_random_element(rng).unwrap();
    let string2 = pool.get_random_element(rng).unwrap();
    let string3 = pool.get_random_element(rng).unwrap();
    let string4 = pool.get_random_element(rng).unwrap();
    GameVariableManager::set_string("CBGM1", string1.as_str());
    GameVariableManager::set_string("CBGM2", string2.as_str());
    GameVariableManager::set_string("CBGM3", string3.as_str());
    GameVariableManager::set_string("CBGM4", string4.as_str());
    chapter.sortie_bgm = string4.into();
    GameSound::field_bgm_set_phase_bgm2(string1.as_str().into(), string2.as_str().into(), string3.as_str().into());
