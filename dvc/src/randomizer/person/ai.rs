    ...
    for order in 0..4 {
        for index in 0..4 {
            ...
            if let Some(person) = ai_value.get_person() {
                if let Some(new_person) = switch_person(person).or_else(|| Some(person)){
...
pub fn adjust_unitai(unit: &mut Unit) {
    ...
    if treasure { unit.private_skill.add_sid("SID_鍵開け", SkillDataCategorys::Private, 0);  }
    ...
    // staff user, Chapter 22 needs to use Force due to Green Emblem Allies
    else if job.get_weapon_mask2().value & ( 1 << 7 ) != 0 {
        ...
        else if unit.has_interference_rod() {
