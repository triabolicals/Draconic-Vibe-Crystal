...
pub fn randomized_emblem_power_spots() {
    if DVCVariables::TerrainEffect.get_value() & 1 == 0 || GameUserData::is_evil_map() { return; }
...
pub fn terrain_spots() {
    ...
    if v < 15 && value > 0 {
        if let Some(tile) = pos_list.get_remove(rng) { 
            MapOverlap::set(tile.0, tile.1, "TID_紋章氣".into(), -1, ForceType::Empty);
        }
    }
    else if v < 50 && value == 2 {
        if let Some(tile) = pos_list.get_remove(rng){
            MapOverlap::set(tile.0, tile.1, TERRAIN[rng.get_value(9) as usize].into(), -1, ForceType::Empty);
        }
    }
    else if v < 80 {
