...
pub fn migrate_to_v2() {
    println!("Migrating Save to Version 2");
    let continious = GameVariableManager::get_number(DVCVariables::CONTINUOUS);
    match continious {
        1 => {
            DVCFlags::ContinuousDLC.set_value(false);
            DVCVariables::Continuous.set_value(1);
        }
        2 => {
            DVCFlags::ContinuousDLC.set_value(true);
            DVCVariables::Continuous.set_value(1);
        }
        3 => {
            DVCFlags::ContinuousDLC.set_value(true);
            DVCVariables::Continuous.set_value(2);
        }
        4 => { DVCVariables::Continuous.set_value(3); }
        5 => { DVCVariables::Continuous.set_value(4); }
        _ => { DVCVariables::Continuous.set_value(0); }
    }

    GameVariableManager::find_starts_with("G_AMPID").iter().for_each(|v|{ GameVariableManager::remove(v.to_string().as_str()); });
    DVCFlags::EngageAttacks.set_value(GameVariableManager::get_number("G_Random_God_Sync") & 2 != 0);
    DVCFlags::EmblemStats.set_value(GameVariableManager::get_number("G_Random_God_Sync") & 1 != 0);
    DVCVariables::ExplorationItem.create_variable(0);
    let sp_mode = GameVariableManager::get_number("G_SPCost");
    if GameVariableManager::get_number("G_Random_Skills") & 1 != 0 { DVCFlags::PersonalSkills.set_value(true); }
    if GameVariableManager::get_number("G_Random_Skills") & 2 != 0 {
        if DVCVariables::JobLearnMode.get_value() == 0 { DVCVariables::JobLearnMode.set_value(1); }
        else {DVCVariables::JobLearnMode.set_value(2);  }
    }
    else if DVCVariables::JobLearnMode.get_value() > 0 { DVCVariables::JobLearnMode.set_value(2); }
    let chaos_mode = GameVariableManager::get_number("G_ChaosMode");
    if GameVariableManager::get_number("G_Random_God_Sync") & 2 != 0 {
        let engage_mode = if chaos_mode & 2 != 0 { 2 } else { 1 };
        DVCVariables::EmblemEngageSkill.create_variable(engage_mode);
        let sync_mode = if chaos_mode & 1 != 0 { 2 } else { 1 };
        DVCVariables::EmblemSyncSkill.set_value(sync_mode);
    }
    DVCFlags::RandomSP.set_value( sp_mode != 0);
    let v = DVCVariables::InteractSetting.get_value();
    match v {
        1 => { DVCVariables::InteractSetting.set_value(2); }
        2 => { DVCVariables::InteractSetting.set_value(3); }
        3 => { DVCVariables::InteractSetting.set_value(1); }
        _ => {}
    }
    let g = DVCVariables::PersonalGrowthMode.get_value();
    let pg_mode = GameVariableManager::get_number("G_PGMode");
    if pg_mode & 2 != 0 { DVCFlags::AdaptiveGrowths.set_value(true); }
    if g & 1!= 0 {
        if pg_mode & 1 != 0 { DVCVariables::PersonalGrowthMode.set_value(2); }
        else { DVCVariables::PersonalGrowthMode.set_value(1); }
    }
    else { DVCVariables::PersonalGrowthMode.set_value(0); }

    DVCFlags::RandomClassGrowth.set_value(g & 2 != 0);
    let rand = GameVariableManager::get_number("G_RandAsset");
    DVCFlags::RandomUnitInfo.set_value(rand & 2 != 0);
    DVCFlags::RandomWeaponAsset.set_value(rand & 1 != 0);
    let new_inherit = if sp_mode != 0 { 1 } else { 0 };
    DVCVariables::EmblemInherit.set_value(new_inherit);
    GameVariableManager::remove("G_RandAsset");
    GameVariableManager::remove("G_Random_Skills");
    GameVariableManager::remove("G_Random_God_Mode");
    GameVariableManager::remove("G_SPCost");
    GameVariableManager::remove("G_Random_Job");
    GameVariableManager::remove("G_HubItems");
    GameVariableManager::remove("G_ChaosMode");
    GameVariableManager::remove("G_PGMode");
    GameVariableManager::set_number("G_DVC_Version", 2);
}
*/
